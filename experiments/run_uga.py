"""
experiments/run_uga.py  [v6 — entropy-based thresholds cho D_high/D_med]
-------------------------------------------------------------------------
...
Usage:
  python experiments/run_uga.py \\
      --data_dir data --max_rounds 3 --mc_passes 20 \\
      --ensemble_seeds 42 123 2024 \\
      --alpha 0.80 --beta 0.40 \\
      --viwordnet path/to/viwordnet.tsv

  # Override diagnostics nếu cần:
  python experiments/run_uga.py [...] --force
"""
...
def partition_by_entropy(
    ...
    ensemble_probs: np.ndarray,  # [N_train, C] ensemble averaged probs
    alpha:          float,        # entropy threshold cho High (vd: 0.80)
    beta:           float,        # entropy threshold cho Med  (vd: 0.40)
) -> Dict[str, pd.DataFrame]:
    """
    Chia training set thành D_high, D_med, D_low dựa trên entropy.

    Phương án B:
      D_high: entropy ≥ alpha (0.80) → augment ×4
      D_med : beta ≤ entropy < alpha  → augment ×2
      D_low : entropy < beta (0.40)   → không augment

    Returns dict với keys 'high', 'med', 'low'.
    """
...
def ensemble_mc_probs(
    ...
    """
    Round 1: Fine-tune 3-seed ensemble và lấy MC-Dropout probs trên training set.

    Paper Algorithm 1, Step 1–2:
      "Fine-tune 3-seed ensemble {M_1, M_2, M_3} on D_train.
       For each x in D_train: compute U(x) = entropy of mean K=20 MC-Dropout
       passes averaged over the 3-seed ensemble."

    Returns
    -------
    np.ndarray : [N_train, C] — ensemble averaged probs trên toàn training set
    """
    import torch
    from torch.utils.data import DataLoader

    val_df   = fold["val"]
    train_df_clean = fold["train"]   # chỉ dùng original training (không augment) cho scoring

    all_seed_probs = []   # [n_seeds, N_train, C]

    for seed in ensemble_seeds:
        print(f"\n  [Ensemble] Training seed {seed}...")
        trainer = PhoBERTTrainer(
            model_name = PHOBERT_BASE,
            lr=2e-5, epochs=5,   # Round 1: 5 epochs đủ để estimate uncertainty
            batch_size=32, max_length=128, patience=3, seed=seed,
            output_dir=os.path.join(output_dir, f"seed_{seed}"),
        )

        # Train trên original data (không augment) để estimate uncertainty
        train_ds = VLSPDataset(train_df_clean["text"], train_df_clean["label"],
                               trainer.tokenizer)
        val_ds   = VLSPDataset(val_df["text"], val_df["label"], trainer.tokenizer)
        trainer.train(train_ds, val_ds)

        # MC-Dropout inference trên training set (để score uncertainty)
        score_ds     = VLSPDataset(train_df["text"], train_df["label"],
                                   trainer.tokenizer)
    ...
    # Trung bình qua ensemble seeds → ensemble probs [N_train, C]
...
# ── Augment D_high và D_med ───────────────────────────────────────────────────

def augment_partition(
    subset_df:    pd.DataFrame,
    factor:       int,          # ×4 cho D_high, ×2 cho D_med
    bt_threshold: float,        # 0.90 cho D_high, 0.85 cho D_med
    ...
    """
    Augment một subset với SR + BT theo factor.

    factor × = SR + BT (vd: ×4 = 2 SR + 2 BT per mẫu gốc)
    Áp dụng class-stratified sampling sau augment (Algorithm 1 Step 4).
    """
    if len(subset_df) == 0:
    ...
    # SR augmentation: n_sr = factor // 2 lần
    n_sr = factor // 2
    ...
    # BT augmentation: n_bt = factor // 2 lần (với threshold cao hơn cho D_high)
    n_bt = factor // 2
    for i in range(n_bt):
    ...
    # Class-stratified sampling để giữ phân phối nhãn (Algorithm 1 Step 4)
    target_n = len(subset_df) * factor
    if len(combined) > target_n:
        # Stratified sample giữ tỉ lệ nhãn
...
def run_uga(args):
    # ── Load fold 0 (đại diện cho UGA experiment) ─────────────────────────────
    print("\nTạo 5-fold splits...")
    folds = create_5fold_splits(data_dir=args.data_dir, n_splits=5)
    fold  = folds[0]   # Dùng fold 0 cho UGA (theo paper: UGA validated on full 5-fold)
    ...
    for round_num in range(1, args.max_rounds + 1):
        ...
        # ── Step 1–2: 3-seed ensemble → entropy scores ────────────────────────
        ...
        # ── Pre-flight diagnostics (C1–C5) ────────────────────────────────────
        # Baseline probs: dùng kết quả từ seed đầu tiên của ensemble
        baseline_probs_r1 = ensemble_probs  # [N_train, C]
        diag = diagnose_uga_conditions(
            baseline_probs  = ensemble_probs,   # dùng training probs làm proxy
        ...
        if not diag["uga_recommended"] and not args.force:
            ...
            print(f"      Dùng --force để tiếp tục bất chấp diagnostics.")
        ...
        aug_med = augment_partition(
            ...
            factor         = 2,
            bt_threshold   = 0.75,   # FIX v6: 0.85→0.75 (ablation shows 0.75 optimal)
            viwordnet_path = args.viwordnet,
            label          = "D_med",
            bt_batch_size  = args.bt_batch_size,
        )
        ...
        # ── Step 5: Re-train trên D_augmented ────────────────────────────────
        ...
        for s in test_report["strata"]:
            print(f"    {s['stratum']}: n={s['n']} ({s.get('pct',0):.1f}%) | "
                  f"acc={s.get('accuracy',0)*100:.1f}%" if s.get('accuracy') else
                  f"    {s['stratum']}: n=0")

        # Cập nhật current_train cho vòng tiếp theo
...
def main():
    ...
    print(f"Stratification: ENTROPY (Phương án B)")
