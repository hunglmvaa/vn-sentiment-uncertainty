...
Key improvements over the previous version:
  ...
  3. Load ViWordNet more robustly from multiple common formats.
  4. Replace only true content tokens with explicit lexical change tracking.
  5. Filter suspicious outputs: unchanged semantics surface-wise, too-short edits,
     malformed punctuation spacing, and duplicate outputs.

Usage:
  python synonym_replacement.py \
...
def normalize_text(text: str) -> str:
    """Normalize whitespace and punctuation spacing for comparison only."""
    text = str(text).strip()
...
def load_viwordnet(path: Optional[str] = None) -> Dict[str, List[str]]:
    """
    Load synonym resources from a flexible set of formats.

    Supported formats:
      1. JSON dict: {"word": ["syn1", "syn2"]}
      2. TSV/CSV: word<TAB>syn1,syn2 OR word,syn1|syn2
      3. ViWordNet-like rows with POS in the first column; these are ignored unless
         a valid lexical key can be extracted safely.

    Falls back to built-in SYNONYM_DICT when no valid file is found.
    """
        ...
        if ext == ".json":
            ...
            with open(path, "r", encoding="utf-8") as f:
                ...
                for row in reader:
                    ...
                    # Ignore POS-like leading columns from WordNet dumps.
                    if first in {"adj", "adv", "noun", "verb", "noun_act", "noun.act", "verb.stative"}:
                        continue

                    if len(row) == 2:
                        parse_synonym_record(first, re.split(r"[,;|]", second), loaded)
                    else:
                        # Treat remaining cells as candidate synonyms.
                        parse_synonym_record(first, row[1:], loaded)
    ...
    print(f"[ViWordNet] Loaded {len(loaded)} external entries; total={len(merged)}")
    return merged
...
class SynonymReplacer:
    ...
    def augment(self, text: str) -> Tuple[str, int]:
        """Return one augmented version of text and the number of lexical changes."""
        tokens = self._pos_tag(str(text))
        eligible_idx = [
            i for i, (w, p) in enumerate(tokens) if self._is_eligible(w, p)
        ]
        if not eligible_idx:
            return str(text), 0

        n_replace = max(1, int(round(len(eligible_idx) * self.rate)))
        replace_idx = set(self.rng.sample(eligible_idx, min(n_replace, len(eligible_idx))))
    ...
    def _is_valid_output(self, src_text: str, aug_text: str, n_changes: int) -> bool:
        if not aug_text.strip():
            return False
        if n_changes < self.min_changes:
            return False
        if is_surface_only_change(src_text, aug_text):
            return False
        if changed_content_tokens(src_text, aug_text) < self.min_changes:
            return False
    ...
    def augment_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        """Augment rows conservatively and preserve source mapping for auditability."""
...
def main() -> None:
    ...
    parser.add_argument("--viwordnet", default=None, help="Path to ViWordNet or synonym resource")
    parser.add_argument("--min_changes", type=int, default=1, help="Minimum lexical changes required")
    args = parser.parse_args()

    print(f"[SR] Rate={args.rate}, seed={args.seed}, min_changes={args.min_changes}")
